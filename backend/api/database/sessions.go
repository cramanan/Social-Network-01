...
// generateB62 generates a random base-64 ID of specified length `n`.
...
func (store *SessionStore) NewSession(w http.ResponseWriter, r *http.Request) *Session {
	session := new(Session)
	session.ID = generateB62(5) // Generates a unique 5-character session ID.
	cookie := http.Cookie{
