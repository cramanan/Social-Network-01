...
const Media = ({ id, username, images, timestamp }: Post) => {
  return (
    <>

      <div className='flex flex-col items-center w-[277px] h-[305px] bg-white rounded-[30px]'>
        <Link href={`/profile`} className='w-[226px] inline-flex items-center gap-3 py-1'>
          <ProfileCircle />
          <div className="text-black text-xl font-extralight font-['Inter']">{username}</div>
        </Link>
        ...
        <div className='w-[226px] flex justify-between gap-5 py-1'>
          <div className="text-black/50 text-[11px] font-extralight font-['Inter']">{timestamp}</div>
          <LikeIcon />
          <CommentIcon />
        </div>
      </div>
